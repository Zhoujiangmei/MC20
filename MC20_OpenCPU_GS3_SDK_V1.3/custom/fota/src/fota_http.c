...
s32 HTTP_FotaMain(u8 contextId, u8* URL)
{
    ...
    ******************************************************************/
    ...
    {
        u32 hostipAddr = 0;
	FOTA_DBG_PRINT("<--  host name is ip string !!! -->\r\n");
...
cuntinuerecvdata:
    ...
    APP_DEBUG("<--want read socket datalength=%d-->\r\n",willrecvlength);
    ...
    if(willrecvlength && (!httpContext_p->bpeersocketclose))
    {
	 ...
	 APP_DEBUG("<--socket read received length = %d -->\r\n", recvlength);
	 APP_DEBUG("<--socket read recvbuffer :%s\r\n",recvbuffer)
/*
	 for(i = 0; i < recvlength; i++)
	 {
	      HexToChar(recvbuffer[i]);
	      APP_DEBUG(" ");
	 }
	 */
        ...
        else
        {
	     ...
	     APP_DEBUG( "<--http head recv dataLen = %d-->\r\n",recvlength); 
        }
    }
    ...
    //if(recvlength > 0)
    {
        ...
        if(iret == HTTP_CODERESULT_END)
        {
	     ...
	     APP_DEBUG("<--httpRecvHttpHead() Get ContentLength= %d-->\r\n",httpContext_p->ContentLength); 
            httpContext_p->getbody = TRUE;
            if(0xFFFFFFFF ==httpContext_p->ContentLength)
            {
                APP_DEBUG("Httphead------RecvHttpChunkedBody");
                iret = http_RecvHttpChunkedBody(httpContext_p->socketid, TRUE);  
                return iret;
            }
            else
            {
...
s8 http_RecvHttpChunkedBody(s32 socketid, bool bContinue)
{
    ...
    FOTA_DBG_PRINT("<--Don't support http chuned encode !!-->\r\n");
     recvlength = Ql_SOC_Recv(httpContext_p->socketid, recvbuffer, 1024);
     //APP_DEBUG("socket read chuned body recvbuffer:%s,recvlength=%d\r\n",recvbuffer,recvlength);
     for(i = 0; i < recvlength; i++)
     	{
     	  APP_DEBUG("%c",recvbuffer[i]);
...
s8 http_RecvHttpChunkedBody(s32 socketid, bool bContinue)
{
    ...
    s32 willrecvlength;
    s32 rev_blockSize = 0;
    s32 blockSize = 0,i,blockSizelen;
    u8 blockHead[10];
    s32 headlen = 0;
    u8 temp_buf[4];
    u8 testBuf[64]={"\r\nQUECTEL_FOTA_UPGRADE"};
    u8 packageFlag = 0;
    u8 *p = NULL;
    APP_DEBUG("genhttp_dstpos=%d\r\n",httpContext_p->genhttp_dstpos)
    //Ql_memcpy(&blockSize, httpContext_p->genhttp_dstconstptr + httpContext_p->genhttp_dstpos , sizeof(s32));
    Ql_memcpy(temp_buf,httpContext_p->genhttp_dstconstptr + httpContext_p->genhttp_dstpos,sizeof(s32));
    for(i = 0; i < 4; i++)
    {
         HexToChar(temp_buf[i]);
         APP_DEBUG(" ");
    }
    ...
    for(i = 0; i < 4; i++)
    {
       ...
       if(temp_buf[i]>='0' && temp_buf[i]<='9') temp_buf[i]=temp_buf[i]-'0';
       else if(temp_buf[i]>='A' && temp_buf[i]<='F') temp_buf[i]=temp_buf[i]-'A'+0x0A;
       else if(temp_buf[i]>='a' && temp_buf[i]<='f') temp_buf[i]=temp_buf[i]-'a'+0x0a;
    }
    blockSize = (temp_buf[0] )*16*16*16 + (temp_buf[1])*16*16 + (temp_buf[2])*16 + temp_buf[3];
    APP_DEBUG("blocksize = %d\r\n",blockSize);
    httpContext_p->genhttp_dstpos +=6; //block
    httpContext_p->genhttp_dstvaliddatalen -= 6;
    rev_blockSize = httpContext_p->genhttp_dstvaliddatalen;
cuntinuerecvbodydata:
    ...
    if(httpContext_p->genhttp_dstpos)
    {
     ...
     APP_DEBUG("genhttp_dstvaliddatalen=%d,genhttp_dstconstsize=%d,genhttp_dstvaliddatalen=%d\r\n",httpContext_p->genhttp_dstvaliddatalen,httpContext_p->genhttp_dstconstsize,httpContext_p->genhttp_dstvaliddatalen);  

     APP_DEBUG("rev_blockSize = %d\r\n",rev_blockSize);
     	 if(rev_blockSize >= blockSize)
	 {
	       //if((rev_blockSize % httpContext_p->genhttp_dstconstsize) == 0)
	       {
	            rev_blockSize = 0;
		     headlen =0;
	            Ql_SOC_Recv(httpContext_p->socketid, blockHead, sizeof(char )*2);
		     APP_DEBUG(" headlen = %d\r\n",headlen);
		     headlen += 2;
		     while(TRUE)
		     {
		         Ql_SOC_Recv(httpContext_p->socketid, &blockHead[headlen], sizeof(char ));
			  if( (0x0a ==blockHead[headlen]) &&   (0x0d ==blockHead[headlen-1]))
			  {
			      break;
			  }
			  headlen++;
			  if(headlen > 10){
			       FOTA_DBG_PRINT("<--download bin file failed -->\r\n");
                            return -1;
			  }
		 ...
		 APP_DEBUG(" headlen = %d\r\n",headlen);
	           for(i = 0; i < headlen;i++)
		   {
		        HexToChar(blockHead[i]);
		        APP_DEBUG(" ");
		   }
	          p = Ql_strstr(&blockHead[2],"\r\n");
		   blockSizelen = p -&blockHead[2];
		   blockSize = 0;
		   APP_DEBUG(" blockSizelen = %d\r\n",blockSizelen);
	         Ql_memcpy(temp_buf,&blockHead[2],blockSizelen);
	        if(blockSizelen == 1){
				blockSize = (temp_buf[0] - 0x30) ;
		}else{
		    for(i = blockSizelen -1; i >= 0; i--)
		    {
		       ...
		       if(temp_buf[i]>='0' && temp_buf[i]<='9') temp_buf[i]=temp_buf[i]-'0';
                       ...
                       APP_DEBUG(" %d\r\n",temp_buf[i]);
			  //blockSize += temp_buf[i] << (16 *<< i);
			  blockSize += temp_buf[i] * (1 << (4 * ( blockSizelen -1 - i)));
                 }
                  //blockSize = (temp_buf[0] )*16*16*16 + (temp_buf[1])*16*16 + (temp_buf[2])*16 + temp_buf[3];
                   //blockSize = (temp_buf[0] -0x30)*16*16*16 + (temp_buf[1] - 0x30)*16*16 + (temp_buf[2] - 0x30)*16 + temp_buf[3] - 0x30;
		}
	        APP_DEBUG("blockSize = %d\r\n",blockSize);
     ...
     else
     {
	  ...
	  recvbuffer = httpContext_p->genhttp_dstconstptr+httpContext_p->genhttp_dstvaliddatalen;
      if((blockSize - rev_blockSize) >= httpContext_p->genhttp_dstconstsize)
      	{
	 ...
	 {
	     willrecvlength =( blockSize - rev_blockSize ) % httpContext_p->genhttp_dstconstsize;
	 }
    ...
    APP_DEBUG("<--socket read body willrecvlength=%d-->\r\n", willrecvlength);
    if(willrecvlength && (!httpContext_p->bpeersocketclose))
    {
        recvlength = Ql_SOC_Recv(httpContext_p->socketid, recvbuffer, willrecvlength);
		/*
        	for(i = 0; i < recvlength;i++)
		{
		     HexToChar(httpContext_p->genhttp_dstconstptr[i]);
		     APP_DEBUG(" ");
		}
		*/
	 APP_DEBUG("<--socket read Chunke body recvlength = %d -->\r\n", recvlength);
	 APP_DEBUG("socket read Chunke body recvbuffer:%s\r\n",recvbuffer);

	APP_DEBUG("genhttp_dstconstptr is:%s\r\n",httpContext_p->genhttp_dstconstptr+httpContext_p->genhttp_dstpos);
	APP_DEBUG("httpContext_p->genhttp_dstvaliddatalen = %d\r\n",httpContext_p->genhttp_dstvaliddatalen);
        if(recvlength == SOC_WOULDBLOCK)
        {
                if(FALSE == httpContext_p->httpGettimerstate)
                {
        ...
        else if(recvlength <= 0)
        {
	     ...
	     APP_DEBUG("<--body http soc_recv == 0-->");
	...
	APP_DEBUG("httpContext_p->genhttp_dstvaliddatalen = %d\r\n",httpContext_p->genhttp_dstvaliddatalen);
        ...
        if(httpContext_p->genhttp_dstvaliddatalen)
        {
             APP_DEBUG("<-- Fota write, genhttp_dstvaliddatalen=%d, genhttp_dstpos=%d-->\r\n",httpContext_p->genhttp_dstvaliddatalen,httpContext_p->genhttp_dstpos);
	    ...
	    APP_DEBUG("<-- Fota write, len=%d, ret=%d, totlesize=%d -->\r\n", httpContext_p->receivedbodydata +httpContext_p->genhttp_dstvaliddatalen, ret, httpContext_p->ContentLength);
            if(ret != 0)
            {
...
cuntinuerecvbodydata:
    ...
    if(willrecvlength && (!httpContext_p->bpeersocketclose))
    {
	 ...
	 APP_DEBUG("<--socket read body recvlength = %d -->\r\n", recvlength);
	 APP_DEBUG("socket read body recvbuffer:%s\r\n",recvbuffer);
        if(recvlength == SOC_WOULDBLOCK)
        {
                if(FALSE == httpContext_p->httpGettimerstate)
                {
        ...
        //Ql_UART_Write(UART_PORT2, httpContext_p->genhttp_dstconstptr+httpContext_p->genhttp_dstpos,httpContext_p->genhttp_dstvaliddatalen );
        APP_DEBUG("genhttp_dstconstptr is:%s\r\n",httpContext_p->genhttp_dstconstptr+httpContext_p->genhttp_dstpos);
	APP_DEBUG("httpContext_p->ContentLength = %d\r\n",httpContext_p->ContentLength);
        /************  write the data to fota temp region*********************/
...
void Httpcallback_socket_read(s32 socketId, s32 errCode, void* customParam )
{
    ...
    APP_DEBUG("<--callback socket read (socketId=%d, errCode=%d)--> \r\n",socketId, errCode);
    ...
    if(NULL == httpContext_p->http_socketdata_p)
    {
	 ...
	 APP_DEBUG(" <--Because Malloc  failed , so exit the socket read !--> \r\n");
        return;
    }
    ...
    else if(0xFFFFFFFF == httpContext_p->ContentLength)// chunked 
    {
        ...
        APP_DEBUG(" <--IN socket read callback  the  ContentLength is chunked !--> \r\n");
        ret = http_RecvHttpChunkedBody(httpContext_p->socketid, TRUE); /**/
    }
    else
    {
        APP_DEBUG(" <--IN socket read callback the RecvHttpBody !--> \r\n");
...
void twobytes2onehex(u8* bytebuf,u8* hexbuf)
{
  ...
  for(k = 0;k < sizeof(hexbuf);k++){
      ...
      if(bytebuf[0]>='0' && bytebuf[0]<='9') temp[0]=bytebuf[0]-'0';
      else if(bytebuf[0]>='A' && bytebuf[0]<='F') temp[0]=bytebuf[0]-'A'+0x0A;
      else if(bytebuf[0]>='a' && bytebuf[0]<='f') temp[0]=bytebuf[0]-'a'+0x0a;
      else return;
    hexbuf[k] = temp[0];
  }
