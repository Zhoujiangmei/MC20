...
int nmea_parser_init(nmeaPARSER *parser)
{
    ...
    if(0 == (parser->buffer = Ql_MEM_Alloc(buff_size)))
...
int nmea_parser_real_push(nmeaPARSER *parser, const char *buff, int buff_sz)
{
    ...
    APP_DEBUG("nmea_parser_real_push\r\n");
    NMEA_ASSERT(parser && parser->buffer);
    ...
    for(;;node = 0)
    {
        ...
        else if(crc >= 0)
        {
            ...
            if(0 == (node = Ql_MEM_Alloc(sizeof(nmeaParserNODE))))
                goto mem_fail;
            ...
            {
            case GPGGA:
                nmea_parse_GGASetHead(GPGGA);
                if(0 == (node->pack = Ql_MEM_Alloc(sizeof(nmeaGPGGA))))
                    goto mem_fail;
                node->packType = GPGGA;
                if(!nmea_parse_GPGGA(
                    (const char *)parser->buffer + nparsed,
                ...
                {
                    Ql_MEM_Free(node->pack);
                    Ql_MEM_Free(node);
                    node = 0;
                }
                break;
            case GPGSA:
                if(0 == (node->pack = Ql_MEM_Alloc(sizeof(nmeaGPGSA))))
                    goto mem_fail;
                node->packType = GPGSA;
                if(!nmea_parse_GPGSA(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GPGSV:
                if(0 == (node->pack = Ql_MEM_Alloc(sizeof(nmeaGPGSV))))
                    goto mem_fail;
                node->packType = GPGSV;
                if(!nmea_parse_GPGSV(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GPRMC:
                nmea_parse_RMCSetHead(GPRMC);
                if(0 == (node->pack = Ql_MEM_Alloc(sizeof(nmeaGPRMC))))
                    goto mem_fail;
                node->packType = GPRMC;
                if(!nmea_parse_GPRMC(
                    (const char *)parser->buffer + nparsed,
                ...
                break;
            case GPVTG:
                if(0 == (node->pack = Ql_MEM_Alloc(sizeof(nmeaGPVTG))))
                    goto mem_fail;
                node->packType = GPVTG;
                if(!nmea_parse_GPVTG(
                    (const char *)parser->buffer + nparsed,
...
int nmea_parser_real_pushs(nmeaPARSER *parser, const char *buff, int buff_sz)
{
    int nparsed = 0, crc, sen_sz, ptype;
    static nmeaParserNODE *node = 0;
    static u8 ucMallocFalg = 0;
    ...
    if(ucMallocFalg == 0){
        ucMallocFalg = 1;
        node = Ql_MEM_Alloc(sizeof(nmeaParserNODE));
        node->pack = Ql_MEM_Alloc(100);
